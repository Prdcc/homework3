...
def analyze(gA=None,gAN=None,gB=None,gBN=None,startSavingAt=5):
    ...
    plt.loglog(epsilon,C,label="C(epsilon) for B")
    plt.loglog(epsilon[start:stop],np.polyval(coeff,epsilon[start:stop]),'--',label="Best fit line for correlation of B, slope={:3.2f}".format(coeff[0]))
    ...
    stop=-15
    coeff=np.polyfit(epsilon[start:stop],C[start:stop],1)
    plt.loglog(epsilon,C,label="C(epsilon) for A")
    plt.loglog(epsilon[start:stop],np.polyval(coeff,epsilon[start:stop]),'--',label="Best fit line for correlation of A, slope={:3.2f}".format(coeff[0]))

    plt.legend()
    plt.xlabel("Epsilon")
    plt.ylabel("C")
    plt.title("Enrico Ancilotto - Generated by analyze\nCorrelation dimension for the two cases")
    plt.savefig("fig"+str(startSavingAt+count)+".png", bbox_inches='tight')
    count+=1
    plt.show()
